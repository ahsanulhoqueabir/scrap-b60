...
OUTPUT_FILE = "output/bd24live_bangla_articles.json"
...
def scrape_bd24live_bangla() -> List[Dict]:
    """
    Scrape articles from BD24Live Bangla RSS feed
    
    Returns:
        List of article dictionaries
    """
    logger.info(f"Starting BD24Live Bangla scraper - fetching RSS feed from {RSS_URL}")
    ...
    try:
        ...
        logger.info(f"Scraped and synced {len(articles)} new articles from BD24Live Bangla")
        return articles
        
    except Exception as e:
        logger.error(f"Failed to scrape BD24Live Bangla: {str(e)}")
        raise


def run():
    """Main entry point for BD24Live Bangla scraper"""
    try:
        # Scrape articles (now syncs to Directus automatically)
        articles = scrape_bd24live_bangla()
        
        # Save to file for backup
        if articles:
            save_json(articles, OUTPUT_FILE)
        
        logger.info(f"BD24Live Bangla scraper completed successfully - {len(articles)} articles processed")
    except Exception as e:
